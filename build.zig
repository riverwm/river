...
pub fn build(b: *Build) !void {
...

    {
        const river_test = b.addTest(.{
            .root_source_file = .{ .path = "river/test_main.zig" },
            .target = target,
            .optimize = optimize,
        });
        river_test.addOptions("build_options", options);
        river_test.addModule("wayland", wayland);
        river_test.linkSystemLibrary("wayland-server");

        river_test.addModule("xkbcommon", xkbcommon);
        river_test.linkSystemLibrary("xkbcommon");

        river_test.addModule("pixman", pixman);
        river_test.linkSystemLibrary("pixman-1");

        river_test.addModule("wlroots", wlroots);
        river_test.linkSystemLibrary("wlroots");

        river_test.linkLibC();
        river_test.linkSystemLibrary("libevdev");
        river_test.linkSystemLibrary("libinput");
        river_test.addModule("flags", flags);

        const run_river_test = b.addRunArtifact(river_test);

        const test_step = b.step("test", "Run the tests");
        test_step.dependOn(&run_river_test.step);
    }
}
